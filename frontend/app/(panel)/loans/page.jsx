...
export default function LoansPage() {
  ...
  const filteredLoans = useMemo(() => {
    ...
    return loans.filter((loan) => {
      ...
      const toolNames = Array.isArray(loan.tools)
        ? loan.tools.map((toolItem) => toolItem?.nombre || '').join(' ').toLowerCase()
        : '';
  ...
  const removeToolFromList = (toolId) => {
    setSelectedTools((prev) => prev.filter((toolItem) => toolItem._id !== toolId));
  };
  ...
  const returnLoan = async (id) => {
    ...
    try {
      ...
      console.error(error);
      toast.error(error.response?.data?.message || 'Error al devolver préstamo');
    }
  };
  ...
  const getModalAvailableTools = () => {
    ...
    const currentToolIds = editingLoan.tools?.map((toolItem) => toolItem._id) || [];
            ...
            >
              ...
              {availableTools.map((toolItem) => (
                <option key={toolItem._id} value={toolItem._id}>
                  {toolItem.nombre} ({toolItem.cantidadDisponible} disponibles de{' '}
                  {toolItem.cantidadTotal})
                </option>
              ))}
            </select>
          </div>
